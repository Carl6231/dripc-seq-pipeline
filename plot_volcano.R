...
# ── identify fold-change and FDR columns ─────────────────────────────────────
...
if (is.null(fc_col) || is.null(fdr_col)) {
  cat("ERROR: Could not find fold-change or FDR columns.\n")
  cat("Available columns:", paste(colnames(df), collapse = ", "), "\n")
  # Still create empty outputs
  pdf(opt$`output-pdf`, width = 7, height = 6)
  plot.new()
  text(0.5, 0.5, "Could not identify FC/FDR columns", cex = 1.2)
  dev.off()
  png(opt$`output-png`, width = 700, height = 600)
  plot.new()
  text(0.5, 0.5, "Could not identify FC/FDR columns", cex = 1.2)
...
# ── classify peaks using FDR ─────────────────────────────────────────────────
df$neg_log10fdr <- -log10(pmax(df[[fdr_col]], .Machine$double.xmin))
df$significance <- "NS"
df$significance[df[[fc_col]] >= log2fc_thresh & df[[fdr_col]] < opt$fdr_threshold] <- "Up"
df$significance[df[[fc_col]] <= -log2fc_thresh & df[[fdr_col]] < opt$fdr_threshold] <- "Down"
...
p <- ggplot(df, aes(x = .data[[fc_col]], y = neg_log10fdr, color = significance)) +
